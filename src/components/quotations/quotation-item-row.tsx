  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-3.5 pb-4 border-b border-slate-100">
        ...
        <div className="flex-1 min-w-0 space-y-2 relative" ref={dropdownRef}>
          <div className="flex flex-wrap items-center justify-between gap-2">
            <div className="flex items-center gap-2">
              <span className="inline-flex items-center justify-center h-5 w-6 rounded-md bg-slate-100 text-[11px] font-mono font-bold text-slate-700 border border-slate-200 shrink-0">
                #{index + 1}
              </span>
            ...
            <div className="flex items-center gap-1.5 shrink-0">
              ...
              <button
                ...
                className="text-[11px] font-bold text-emerald-700 hover:text-emerald-800 flex items-center gap-1 cursor-pointer bg-emerald-50 hover:bg-emerald-100 px-2.5 py-1.5 sm:py-1 rounded-lg border border-emerald-200/80 shadow-2xs transition-colors min-h-[34px] sm:min-h-0"
              >
              ...
              <button
                ...
                className="text-[11px] font-semibold text-slate-500 hover:text-slate-800 flex items-center gap-1 cursor-pointer bg-slate-100 hover:bg-slate-200/70 px-2.5 py-1.5 sm:py-1 rounded-lg border border-slate-200 transition-colors whitespace-nowrap min-h-[34px] sm:min-h-0"
              >
                {showDetails ? (
                  <>
                    Hide Notes <ChevronUp className="h-3 w-3 shrink-0" />
                  </>
                ) : (
                  <>
                    + Notes / HSN <ChevronDown className="h-3 w-3 shrink-0" />
                  </>
                )}
              </button>
            </div>
          ...
          {/* Autocomplete Suggestions Popup - Viewport Safe on Mobile */}
          {isDropdownOpen && (suggestions.length > 0 || (searchFilter && !isExistingInCatalog)) && (
            <div className="absolute left-0 right-0 sm:right-auto w-full sm:w-[130%] sm:min-w-[320px] max-w-[calc(100vw-3rem)] sm:max-w-[500px] top-full mt-2 z-50 rounded-2xl bg-white p-2.5 border border-slate-200 shadow-2xl animate-in fade-in-50 zoom-in-95 backdrop-blur-md overflow-hidden">
        ...
        {/* Right: Amount & Actions (Mobile Touch Optimized) */}
        <div className="flex items-center justify-between sm:flex-col sm:items-end sm:justify-start gap-2 sm:gap-1 shrink-0 pt-2 sm:pt-0 sm:pl-3 border-t border-slate-100/70 sm:border-0">
          <div className="flex items-center gap-2 sm:w-full sm:justify-end">
            ...
            {isRemovable && (
              <button
                ...
                className="text-slate-400 hover:text-rose-600 hover:bg-rose-50 p-2 sm:p-1 rounded-xl transition-colors cursor-pointer min-h-[36px] min-w-[36px] sm:min-h-0 sm:min-w-0 flex items-center justify-center"
                title="Remove Item"
          ...
          <div className="w-36 sm:w-44">
            <input
              ...
              className="w-full text-right rounded-xl border border-slate-200 bg-slate-50/50 hover:bg-white focus:bg-white px-3 py-2 sm:py-2 text-base font-mono font-extrabold text-slate-900 focus:border-emerald-500 focus:outline-none focus:ring-2 focus:ring-emerald-500/20 shadow-2xs transition-all"
            />
          </div>
        </div>
      </div>
      ...
      {/* ========================================================================= */}
      <div className="pt-3.5 grid grid-cols-2 sm:grid-cols-4 gap-2.5 sm:gap-4 items-start">
        {/* Col 1: Quantity & Unit */}
        <div className="space-y-1">
          <div className="flex items-center justify-between">
            <label className="text-xs font-bold text-slate-600">Qty & Unit</label>
            {item.unit && (
              <span className="text-[10px] font-extrabold text-emerald-800 bg-emerald-100/90 px-1.5 py-0.2 rounded truncate max-w-[60px]">
                {item.unit}
              </span>
            )}
          </div>
          <div className="flex items-center gap-1">
            <input
              ...
              className="w-1/2 rounded-xl border border-slate-200 bg-white px-2 py-2 text-xs sm:text-sm font-semibold text-slate-900 placeholder:text-slate-400 focus:border-emerald-500 focus:outline-none focus:ring-2 focus:ring-emerald-500/20 shadow-2xs h-10 sm:h-9"
            />
            <select
              ...
              className="w-1/2 rounded-xl border border-slate-200 bg-slate-50/90 px-1 py-2 text-xs font-bold text-slate-800 focus:bg-white focus:border-emerald-500 focus:outline-none focus:ring-2 focus:ring-emerald-500/20 cursor-pointer shadow-2xs h-10 sm:h-9"
            >
          ...
          {isCustomUnit && (
            <input
              ...
              className="w-full mt-1 rounded-lg border border-slate-200 bg-white px-2.5 py-1.5 text-xs font-semibold text-slate-800 placeholder:text-slate-400 focus:border-emerald-500 focus:outline-none"
              autoFocus
            />
          )}
        </div>
        ...
        <div className="space-y-1">
          ...
          <input
            ...
            className="w-full rounded-xl border border-slate-200 bg-white px-2.5 py-2 text-xs sm:text-sm font-semibold text-slate-900 placeholder:text-slate-400 focus:border-emerald-500 focus:outline-none focus:ring-2 focus:ring-emerald-500/20 shadow-2xs h-10 sm:h-9"
          />
        </div>
        ...
        <div className="space-y-1">
          <div className="flex items-center justify-between">
            ...
            {Boolean(item.discountValue && item.discountValue > 0) && (
              <span className="text-[10px] font-extrabold text-emerald-700 bg-emerald-100/90 px-1.5 py-0.2 rounded truncate max-w-[65px]">
          ...
          <div className="flex items-center gap-1">
            <input
              ...
              className="w-3/5 rounded-xl border border-slate-200 bg-white px-2 py-2 text-xs sm:text-sm font-semibold text-slate-900 focus:border-emerald-500 focus:outline-none focus:ring-2 focus:ring-emerald-500/20 shadow-2xs h-10 sm:h-9"
            />
            <select
              value={item.discountType || "percentage"}
              onChange={(e) => handleDiscountTypeChange(e.target.value as "percentage" | "fixed")}
              className="w-2/5 rounded-xl border border-slate-200 bg-slate-50/90 px-1 py-2 text-xs font-bold text-slate-800 cursor-pointer focus:outline-none shadow-2xs h-10 sm:h-9"
            >
        ...
        <div className="space-y-1">
          ...
          <select
            ...
            className="w-full rounded-xl border border-slate-200 bg-white px-2.5 py-2 text-xs sm:text-sm font-semibold text-slate-900 focus:border-emerald-500 focus:outline-none focus:ring-2 focus:ring-emerald-500/20 cursor-pointer shadow-2xs h-10 sm:h-9"
          >
